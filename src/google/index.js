...
const createBrowserPage = async (browser) => {
  ...
  page.setDefaultTimeout(10000); // For other operations like waitForSelector
...
export const runGoogleBrowserInstance = async (message, callback) => {
  let browser;
  ...
  try {
    ...
    return await message.reply('Could not fetch data. Google may be causing issues.');
  } finally {
    await browser?.close();
  }
};
